...
def _specified_resources(
    ...
    *,
    ...
    output("SLURM 分区：" + ", ".join(partitions))
    partition = input_fn("分区名 >> ").strip()
    if partition not in partitions:
        raise ValueError(f"未知 SLURM 分区：{partition}")
    nodes = scheduler.inspect_nodes(partition)
    output(f"{partition} 节点：")
    for item in nodes:
        output(
            f"  {item.name}: state={item.state} total={item.total_cores} "
            f"allocated={item.allocated_cores} idle={item.idle_cores} other={item.other_cores}"
        )
    selected_name = input_fn("节点名 >> ").strip()
    selected = next((item for item in nodes if item.name == selected_name), None)
    if selected is None:
        raise ValueError(f"节点 {selected_name} 不属于分区 {partition}")
    state = selected.state.lower()
    if any(flag in state for flag in ("down", "drain", "fail", "maint", "unknown", "reserved")):
        raise ValueError(f"节点当前不可用：{selected_name}")
    tasks = _positive_int("任务数 >> ", input_fn)
    if selected.idle_cores < tasks:
        raise ValueError(
            f"节点 {selected_name} 空闲核心数 {selected.idle_cores} 小于请求值 {tasks}"
        )
    persist = (
    ...
    return ResourceRequest.create(
        ...
        partition=partition, nodes=(selected_name,), node_count=1,
        tasks=tasks, tasks_per_node=tasks,
        walltime=config.scheduler.walltime,
        script=config.scheduler.script,
        persist=persist,
    )
    ...
    while True:
        ...
        try:
            if choice == "2":
                ...
                return ResourceRequest.create(
                    allocation="auto",
                    partition=config.scheduler.partition, nodes=(),
                    node_count=config.scheduler.node_count,
                    tasks=tasks, tasks_per_node=config.scheduler.tasks_per_node,
                    walltime=config.scheduler.walltime,
                    script=config.scheduler.script,
                    persist=persist,
                )
...
def resource_cli_argv(resources: ResourceRequest) -> List[str]:
    ...
    argv.extend(["--resource-node-count", str(resources.node_count)])
    argv.extend(["--resource-tasks", str(resources.tasks)])
    argv.extend(["--resource-tasks-per-node", str(resources.tasks_per_node)])
